...
const DeviceOrientationMixin = (superclass, ...eventNames) => class extends superclass {
  ...
  [deactivateCallback]() {
    // MP 2019-03-04 this removeEventListener instruction does not seem to work
...
class AbsoluteOrientationSensor extends DeviceOrientationMixin(
  ...
  [handleEventCallback](event) {
    ...
    if (initialBearingAngle === null && event.webkitCompassHeading !== 0) {
      initialBearingAngle = (-event.webkitCompassHeading - event.alpha + 720) % 360
    }
    
    const heading = event.webkitCompassHeading != null ? (initialBearingAngle + event.alpha) % 360 : event.alpha;
