  ...
  methods: {
    ...
    async submit() {
      ...
      if (this.$v.form.$error) {
        this.$q.notify({type: 'negative', message: this.$t('message.StepSettingsFormError')})
        return
      }
      ...
      this.$q.notify({type: 'positive', message: this.$t('message.StepSaved')})
    ...
    addAnswer: function () {
      if (this.form.answers.length >= this.maxNbAnswers) {
        this.$q.notify({type: 'negative', message: this.$t('message.YouCantAddMoreThanNbAnswers', { nb: this.maxNbAnswers })})
      } else {
    ...
    deleteAnswer: function (key) {
      if (this.form.answers.length <= this.minNbAnswers) {
        this.$q.notify({type: 'negative', message: this.$t('message.YouMustDefineAtLeastNbAnswers', { nb: this.minNbAnswers })})
      } else {
