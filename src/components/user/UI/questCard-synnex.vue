<template>
  <q-card class="my-card" :style="cssVars" @click="!info ? $router.push('/quest/play/' + quest.questId) : ''" :class="info ? '' : 'cursor-pointer'">
    ...
    <q-card-section class="q-pa-none relative-position section-up" :style="'background: url(' + getThumbImage() + ' ) center center /cover no-repeat; height: 250px;'">
      ...
      <div class="inter-bar" style="background-color: rgb(0,87,88)" :class="direction === 'left' ? 'left' : 'right'"></div>
      <creator-badge class="creator-badge" :class="direction === 'left' ? 'left' : 'right'" style="background-color: #005758" :color="'rgb(0,87,88)'" :creator="creator" :custom="customization"/>
...
<!--        <img src="statics/images/icon/point.png" />-->
    ...
    <q-card-section v-if="info" class="section-info" style="background-color: rgb(0,87,88)">
      ...
      <div v-if="isUserTooFar && !quest.allowRemotePlay" class="q-pt-md" style="background-color: rgb(0,87,88)">
        <q-icon color="secondary" name="warning" />&nbsp; <span v-html="$t('label.QuestIsFarFromUser')" />
      </div>
    </q-card-section>
    <!--   INFO QUEST PLAY  -->
    <q-card-section v-if="info" class="section-down" style="background-color: rgb(0,87,88)">
